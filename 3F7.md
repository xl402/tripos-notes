...
M = P(X\geq a)
$$
We can write down with confidence that $\mu \geq Ma$ since at best we have $M$ being a point mass that lies exactly $\mu$, otherwise it will bring the center of mass up. Therefore we arrive at the Markov's inequality:
$$
